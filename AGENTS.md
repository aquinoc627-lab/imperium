...
- Allowed product: through Phase 19 (complete) — see `specs/STATUS.md`. Specs `07`–`19` and `specs/v0-slice.md` describe what shipped. Specs `02`–`04` are deferred depth; do not implement them.
- Next candidates named by the owner: Monte Carlo factor breadth. Needs its own named spec before work starts.
