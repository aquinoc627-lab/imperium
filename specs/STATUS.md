...
Phases 19–24 shipped (voice input-only; MC factor breadth; host-layer
hardening; release & distribution; MCP packaging; CI governance). Specs
`02`–`04` retain deferred depth (causal graphs, A/B auto-promotion, LLM
patching, P2P, offline/on-device STT, TPM/SLSA) that would each need a
new named spec. SLSA Levels 2–4 and sigstore signing remain deferred even
after Phase 22 (provenance is emitted, not signed).
